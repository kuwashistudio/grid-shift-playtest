...
EXPECTED_BUNDLE_BLOB="74ac6b6e0cddbce738c6679b37961eaa5159f791"
...
def main():
    ...
    req("cars.set(c.id,c);sprite(c)" in html,"initial production sprites are not mounted from Level Data")
    ...
    req('id="hintPulse"' in html,"dynamic first-focus hint missing after baked tutorial removal")
    ...
    req(git_blob(LOGIC)==EXPECTED_LOGIC_BLOB,"game_logic.js changed during visual migration")
    req(git_blob(BUNDLE)==EXPECTED_BUNDLE_BLOB,"levels.generated.js changed during visual migration")
    req(git_blob(LEVEL)==EXPECTED_LEVEL_BLOB,"level_001.json changed during visual migration")

    print("PASS VP-2G migration structure: clean plate + persistent production sprites; no patch/reveal; logic/data unchanged")
    return 0
