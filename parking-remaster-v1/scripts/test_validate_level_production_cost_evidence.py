...
def good(level="L01",run="r1",fingerprint=None):
    if fingerprint is None:
        fingerprint=(level.encode().hex()+"0"*64)[:64]
    return {"level_id":level,"authoring_run_id":run,"started_at":"2026-09-22T00:00:00+00:00","finished_at":"2026-09-22T00:10:00+00:00","stage_seconds":{"design":60,"level_data":90,"solver_parity":30,"visual_assembly":90,"qa":60,"human_review_rework":30},"rework_cycles":1,"human_review_seconds":30,"tool_compute_seconds":20,"result":"PASS","content_fingerprint":fingerprint}
...
out=v.validate_batch(batch); assert out["median_active_production_seconds"]==360 and out["p90_active_production_seconds"]==360 and out["distinct_pass_content_fingerprints"]==5
...
copied=[good(f"COPY{i}",f"r{i}","a"*64) for i in range(1,6)]
try: v.validate_batch(copied)
except ValueError as e: assert "distinct PASS content fingerprints" in str(e)
else: raise AssertionError("renamed/copied logical content must not prove repeatability")
