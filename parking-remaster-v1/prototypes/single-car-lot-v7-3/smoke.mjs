...
await page.waitForFunction(()=>window.__v73QA);
const initial=await page.evaluate(()=>window.__v73QA.state());
await page.evaluate(()=>window.__v73QA.start());

await page.waitForTimeout(390);
let q=await page.evaluate(()=>({s:window.__v73QA.state(),k:window.__v73QA.skidStats(),a:window.__v73QA.audioState()}));
...
A(q.a.kind==='jittered-combustion-roar+tire-squeal-noise','beep oscillator engine was not replaced');
A(q.a.metrics.rms>.005,'audio output too weak');
A(q.a.target.roar>.05,'engine roar layer missing');
await page.screenshot({path:'01-charge-audio.png'});

await page.waitForFunction(()=>window.__v73QA.state().phaseName==='snap',{timeout:1000});
await page.waitForTimeout(118);
q=await page.evaluate(()=>({k:window.__v73QA.skidStats(),d:window.__v73QA.skidData(),a:window.__v73QA.audioState()}));
...
A(q.a.target.tire>=.35,'snap tire squeal too weak');
await page.screenshot({path:'02-smooth-snap-arcs.png'});

await page.waitForFunction(()=>window.__v73QA.state().phaseName==='reload',{timeout:1000});
await page.waitForTimeout(180);
q=await page.evaluate(()=>({k:window.__v73QA.skidStats(),a:window.__v73QA.audioState()}));
A(q.k.reloadLeft===0&&q.k.reloadRight===0,'reload created fake skid');
await page.screenshot({path:'03-reload-clean.png'});

await page.waitForFunction(()=>window.__v73QA.state().done===true,{timeout:1400});
const done=await page.evaluate(()=>({s:window.__v73QA.state(),k:window.__v73QA.skidStats(),a:window.__v73QA.audioState()}));
...
const mid=await page.evaluate(()=>window.__v73QA.skidStats());
A(mid.leftStrokes>0&&mid.rightStrokes>0,'real skids faded too fast');
await page.waitForTimeout(3300);
const end=await page.evaluate(()=>window.__v73QA.skidStats());
